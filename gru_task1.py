...
def train(target_var,predictor,optimizer,criterion,max_length=MAX_LENGTH):
    ...
    for si in range(target_length-1):
        ...
        # if target_var[si].data[0]==0 and target_var[si+1].data[0]!=0:
...
def train_iters(predictor,corpus,lines,n_iters,print_every=1000,plot_every=100,learning_rate=1e-3):
    start=time.time()
    plot_losses=[]
    print_loss_total=0
    plot_loss_total=0
...
def eval(predictor,corpus,lines):
    ...
    with open('data/res.txt','w') as f:
        for line in lines:
            ...
            f.write('tar:%s, pred:%s, correct:%d\n' % (''.join(tar),''.join(pred),int(nc==nt)))
            ncorr+=nc
            ntotal+=nt
    print(ncorr/ntotal)

if __name__=='__main__':
    corpus,lines_train=prepare_data('corpus_train_t1')
    ...
    train_iters(predictor,corpus,lines_train,1000,print_every=20,plot_every=20)

    _,lines_test=prepare_data('corpus_test_t1')
